	...
	public function controller_methods($default_value, $template_datas, $arguments, $callback_format_value, $current_file_content, $file_path, $all_keywords){
		...
		// INDEX  START  .......
			$datas['index'] = [
				...
				) . "(\n\t\t" . $repository_this_property . "->getPaginate(30)\n\t)") : 'view(' . (isset($datas['index']['view']) ? $datas['index']['view'] : '"pages.' . Str::plural($model_property) . '.index"') . ", ['datas' => " . (isset($datas['index']['view.datas']) ? $datas['index']['view.datas'] : '$' . $model_property) . '])')
			];	
		// INDEX  STOP  .......
		
		// CREATE  START  .......
			if(!$is_api || ($is_api && isset($datas['create']))){
				$model_f_create = (function($tab, $model_namespace){
					$t = ['class' => [], 'datas' => []];
					...
					return $t;
				})($this->colone($arguments['columns'], false, false), $model_namespace);

				$datas['create'] = [
					'type_returned' => isset($datas['create']['type_returned']) ? $datas['create']['type_returned'] : ($is_api  ? null : null),
					'args' => isset($datas['create']['args']) ? $datas['create']['args'] : array_merge([
						$model_property => $model_full_class
						// $model_property => $model_full_class
					], $model_f_create['class']),
					...
					// 'return' => isset($datas['create']['return']) ? $datas['create']['return'] : ($is_api ? null : 'view(' . (isset($datas['create']['view']) ? $datas['create']['view'] : '"pages.' . Str::plural($model_property) . '.create"') . ", " . (isset($datas['create']['view.datas']) ? $datas['create']['view.datas'] : '$' . $model_property) . ')')
					'return' => isset($datas['create']['return']) ? $datas['create']['return'] : ($is_api ? null : 'view(' . (isset($datas['create']['view']) ? $datas['create']['view'] : '"pages.' . Str::plural($model_property) . '.create"') . ", ['fillable' => $" . $model_property . "->getFillable()" . (!empty($model_f_create['datas']) ? (', ' . implode(', ', $model_f_create['datas'])) : null) . "])")
				];
			}
		// CREATE  STOP  .......
		...
		// STORE  START  .......
			...
			if(((!isset($datas['store']['return']) && $is_api) || (!$is_api && !isset($datas['store']['code']))) && isset($datas['store']['with']) && (is_string($datas['store']['with']) || is_array($datas['store']['with']) && !empty($datas['store']['with'])) ){
				$with = is_string($datas['store']['with']) ? [$datas['store']['with'] => []] : $this->colone($datas['store']['with'], false, false, []);
				$rec = [];
				...
				$ret = [];
				foreach ($this->colone($arguments['columns'], false, false) as $key => $value) {
		...
		// SHOW  START  .......
			$datas['show'] = [
				...
				'args' => isset($datas['show']['args']) ? $datas['show']['args'] : ($is_api ? [
					$model_property => $model_full_class
				] : ['id', $model_property => $model_full_class]),
				...
				) . "(\n\t\t" . $repository_this_property . "->getById($" . $model_property . "->id)\n\t)") : 'view(' . (isset($datas['show']['view']) ? $datas['show']['view'] : '"pages.' . Str::plural($model_property) . '.show"') . ", ['datas' => " . (isset($datas['show']['view.datas']) ? $datas['show']['view.datas'] : '$' . $model_property) . '->find($id)])')
			];
		// SHOW  STOP  .......
		...
		// EDIT  START  .......
			if(!$is_api || ($is_api && isset($datas['edit'])))
				$datas['edit'] = [
					...
					'return' => isset($datas['edit']['return']) ? $datas['edit']['return'] : ($is_api ? null : 'view(' . (isset($datas['edit']['view']) ? $datas['edit']['view'] : '"pages.' . Str::plural($model_property) . '.edit"') . ", ['datas' => " . (isset($datas['edit']['view.datas']) ? $datas['edit']['view.datas'] : '$' . $model_property) . '])')
				];
		// EDIT  STOP  .......
		...
		// ALL  GET FIND START .......
			$dt = $this->getMigrationDatas($arguments, $this, $this->template_config['foreign_default']);
			$search_code = [];
			...
			if(!$is_api){
				$datas['find'] = [
					...
					'return' => isset($datas['find']['return']) ? $datas['find']['return'] : 'view(' . (isset($datas['find']['view']) ? $datas['find']['view'] : '"pages.' . Str::plural($model_property) . '.find"') . ", ['find' => \$find, 'type' => \$type])",
				];
			}
		// ALL  GET FIND STOP .......
		...
		// DESTROY  START  .......
			...
			if(!isset($datas['destroy']['code']) && isset($datas['destroy']['with']) && (is_string($datas['destroy']['with']) || is_array($datas['destroy']['with']) && !empty($datas['destroy']['with'])) ){
				$with = is_string($datas['destroy']['with']) ? [$datas['destroy']['with'] => []] : $this->colone($datas['destroy']['with'], false, false, []);
				$rec = [];
				// repos__ $args_ = ['id'];
				foreach ($with as $model => $value) {
